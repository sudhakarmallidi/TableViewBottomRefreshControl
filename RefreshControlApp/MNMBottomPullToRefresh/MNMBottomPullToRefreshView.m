...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        iconImageView_ = [[UIImageView alloc] initWithFrame:CGRectMake(220.0f, round(CGRectGetHeight(frame) / 2.0f) - round([iconImage size].height / 2.0f), [iconImage size].width, [iconImage size].height)];
        iconImageView_.backgroundColor = [UIColor redColor];
...
- (void)changeStateOfControl:(MNMBottomPullToRefreshViewState)state offset:(CGFloat)offset {
    ...
    switch (state_) {
        ...
        case MNMBottomPullToRefreshViewStateIdle: {
            ...
            [messageLabel_ setText:MNM_BOTTOM_PTR_LOADING_TEXT_KEY];//MNM_BOTTOM_PTR_RELEASE_TEXT_KEY
